...
const [submitButton, setSubmitButton] = useState(false);
  ...
  setSelectedAnswers((prev) => {
    const alreadyAnswered = prev[currentIndex];
  ...
  return (
    <>
    <span className=' inline-block mb-2'>Questions : {currentIndex + 1}/{questions.length}</span>
      <h2 className="text-2xl bg-green-900 p-2 rounded-md">
        ...
        {questions[currentIndex].options.map((option, idx) => (
          ...
          ><span>{idx+1}</span>
            ...
            <span className=" bg-green-600 rounded-full ">
              <IoCheckmarkDoneCircle size={20} className={` ${currentAnswer === option ? "block ": "hidden "} `}/>
            </span>
          </button>
        ))}
      </div>
